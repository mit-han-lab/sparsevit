...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, w_index=None):
        ...
        """
        B, nW, N, C = x.shape
        qkv = self.qkv(x).reshape(B*nW, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        if mask is not None:
            ...
            attn = attn.view(B, nW, self.num_heads, N, N) + mask.unsqueeze(2)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x_windows, mask_matrix, pad_shape, w_index):
        """ Forward function.
        ...
        x_windows = res_identity + self.drop_path(attn_windows)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if not shift:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, query_windows, attn_mask, pad_shape, w_index)
                else:
                    x = blk(query_windows, attn_mask, pad_shape, w_index)
            else:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, query_windows, attn_mask, pad_shape, shift_w_index)
                else:
                    x = blk(query_windows, attn_mask, pad_shape, shift_w_index)
